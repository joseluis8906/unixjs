...
Gwt.Core.Gwdm = function ()
{
    ...
    var method = sessionStorage.getItem("user") ? "Renew" : "Terminate";
    Rpc.Send ({Method: method}, function(Res) {
        if ( method === "Renew" && Res.Result === 1) {
            login.close ();
            gpanel.open ();
            gcontrol.open ();
            ...
            window.addEventListener ("mousemove", function (){if (TerminateSession !== undefined) {clearTimeout (TerminateSession); TerminateSession = setTimeout (Gwt.Core.TerminateSession, 60000 * 5); }});
            window.addEventListener ("keypress", function (){if (TerminateSession !== undefined) {clearTimeout (TerminateSession); TerminateSession = setTimeout (Gwt.Core.TerminateSession, 60000 * 5); }});
...
{
    var oHiddFrame = window.open(sURL);//document.createElement("iframe");
    //oHiddFrame.onload = oHiddFrame.print;
...
Gwt.Core.Session = function (UserName, Password)
{
    ...
    */
    var Rpc = new Gwt.Core.Rpc ("/session/");
    Rpc.Send ({Method: "Terminate", UserName: UserName, Password: Password}, this.Ready.bind(this));
};
...
Gwt.Gui.Entry.prototype.TextAlign = function (Value)
{
...
Gwt.Gui.SelectBoxItem = function (Text, Value)
{
    ...
    this.SetHeight (24);
    var background_color = new Gwt.Gui.Contrib.Color (Gwt.Gui.Contrib.Colors.Transparent);
...
Gwt.Gui.SelectBox.prototype.SetValueListener = function (Event, Text, Value)
{
    for (var i = 0; i < this.Options.length; i++)
    {
	    ...
	    {
            this.Options [i].SetBackgroundImage (null);            
	    }
    }
};
...
Gwt.Gui.IconEntry.prototype.TextAlign = function (Value)
{
...
Gwt.Gui.Date = function (Placeholder)
{
    ...
    this.month.ConnectChangedEvent (this.UpdateDays.bind(this));
...
Gwt.Gui.Date.prototype._Date = function ()
{
...
Gwt.Gui.Date.prototype.SetWidth = function (Width)
{
    this.Width = Width;
    this.SetMaxWidth (this.Width);
    this.SetMinWidth (this.Width);
...
Gwt.Gui.Date.prototype.UpdateDays = function ()
{
    var NumDays = Gwt.Core.Contrib.DaysInMonth(Number(this.month.GetText()), Number(this.year.GetText()));
    var days_items = [];
    ...
    this.day.SetOptionsList (days_items);
    this.day.SetText (1);
}
//Ends Gwt::Gui::Image
//##################################################################################################
